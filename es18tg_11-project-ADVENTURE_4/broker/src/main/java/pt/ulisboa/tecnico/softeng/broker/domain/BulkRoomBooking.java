...
public class BulkRoomBooking extends BulkRoomBooking_Base {
	...
	public BulkRoomBooking(Broker broker, int number, LocalDate arrival, LocalDate departure, String buyerNif,
			String buyerIban) {
		checkArguments(number, arrival, departure, buyerNif, buyerIban);
		...
		setBroker(broker);
		broker.addRoomBulkBooking(this);
	}
	...
	private void checkArguments(int number, LocalDate arrival, LocalDate departure, String buyerNif, String buyerIban) {
		if (number < 1 || arrival == null || departure == null || departure.isBefore(arrival) || buyerNif == null || buyerIban == null) {
