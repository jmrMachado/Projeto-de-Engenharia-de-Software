...
public class BrokerPersistenceTest extends BaseTest {
	...
	public void atomicProcess() {
		...
		new Adventure(this.begin, this.end, client,  AMOUNT,MARGIN, true);
	...
	public void atomicAssert() {
		...
		List<Adventure> adventures = new ArrayList<>(broker.getClientByNIF(CLIENT_NIF).getAdventureSet());
		Adventure adventure = adventures.get(0);

		assertNotNull(adventure.getID());
		assertEquals(broker, adventure.getClient().getBroker());
        ...
        assertEquals(AMOUNT, adventure.getCurrentAmount(), 0);
